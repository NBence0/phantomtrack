...
/**
 * Ez a szkript a "Trackerek" (korábban Tokenek) adminisztrációs oldal dinamikus funkcióit kezeli.
 * VERZIÓ FRISSÍTVE: Már a `TrackerPageConfig`-et használja, és az AJAX kéréshez hozzáadja a tracker típusát.
 *
 * Függőségek:
 *  - Egy globális 'TrackerPageConfig' objektum.
 *  - `showDynamicMessage`, `copyToClipboard` globális funkciók.
 */

// A felhasználó összes kategóriája, amit a PHP-ből kapunk
const allUserCategories = TrackerPageConfig.allUserCategories;

// ===================================================================
// ÚJ TRACKER LÉTREHOZÁSÁNAK KEZELÉSE
// ===================================================================
const addTokenForm = document.getElementById('addTokenForm');
if (addTokenForm) {
    addTokenForm.addEventListener('submit', function(e) {
        e.preventDefault();
        const formData = new FormData(this);
        const submitButton = this.querySelector('button[type="submit"]');
        const originalButtonText = submitButton.innerHTML;
        ...
        submitButton.disabled = true;
        submitButton.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Létrehozás...';

        fetch(TrackerPageConfig.ajaxUrl, {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
        ...
        .catch(error => {
            showDynamicMessage('Hálózati hiba történt a tracker létrehozásakor.', 'error');
        })
        .finally(() => {
            submitButton.disabled = false;
            submitButton.innerHTML = originalButtonText;
...
// ===================================================================
// TRACKER SZERKESZTÉSÉNEK LOGIKÁJA
// ===================================================================
function openEditTokenModal(tokenId) {
    // Ez a függvény nagyrészt változatlan, a TrackerPageConfig használata a kulcs.
    const modal = document.getElementById('editTokenModal');
    const form = document.getElementById('editTokenForm');
    const trigger = form.querySelector('.select-trigger');
    ...
    .then(data => {
        if (data.success && data.token) {
            ...
            if (!token.category_id) noCatSpan.classList.add('selected');
            trigger.textContent = selectedCategoryName;
        } else {
            alert('Hiba: ' + (data.message || 'A tracker adatok betöltése sikertelen.'));
            modal.style.display = 'none';
        }
    })
    .catch(error => {
...
// ===================================================================
// ÚJ KATEGÓRIA HOZZÁADÁSA (logika változatlan)
// ===================================================================
function handleAddNewCategory() {
    const newCategoryNameInput = document.getElementById('new_category_name_modal');
    const newCategoryName = newCategoryNameInput.value.trim();
    const responseEl = document.getElementById('category_ajax_response');
    const csrfToken = document.getElementById('addTokenForm').querySelector('input[name="csrf_token"]').value;

    if (newCategoryName === '') {
        responseEl.textContent = 'Kérlek, adj meg egy nevet!';
        responseEl.style.color = 'var(--color-warning)';
        return;
    }
    responseEl.textContent = 'Feldolgozás...';
    ...
    .then(r => r.json()).then(data => {
        if (data.success && data.category) {
            ...
            // Kiválasztjuk az újonnan hozzáadott kategóriát
            // A globális `handleOptionClick` esemény-delegálással működik, szimuláljuk a kattintást
            newCustomOption.click(); 
...
// ===================================================================
// PIXEL KÓD MODÁLIS ABLAK (logika változatlan)
// ===================================================================
function openGetCodeModal(tokenValue) {
    const pixelUrl = `${TrackerPageConfig.baseUrl}pixel.php?token=${tokenValue}`;
    document.getElementById('snippet-html').textContent = `<img src="${pixelUrl}" alt="pixel" width="1" height="1" border="0">`;
    document.getElementById('snippet-markdown').textContent = `![pixel](${pixelUrl})`;
    document.getElementById('snippet-bbcode').textContent = `[img]${pixelUrl}[/img]`;
    Prism.highlightAllUnder(document.getElementById('getCodeModal'));
